package WebGUI::Macro::RecentPostsOelan;
...
It returns a loop (<tmpl_loop postLoop>a) containting tmpl_vars for each of the retrieved threads.
...
sub process {
    ...
    my $threads		= $parent->getLineage( [ 'descendants' ], {
		...
		orderByClause		=> 'lastPostDate DESC'
    });

    foreach my $thread ( @$threads ) {
		my $post	= WebGUI::Asset->newByDynamicClass( $session, $thread->get( 'lastPostId' ) );
