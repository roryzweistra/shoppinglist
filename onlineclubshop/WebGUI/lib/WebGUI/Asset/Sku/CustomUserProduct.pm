...
sub getAddToCartForm {
    ...
    COLLATERAL: foreach my $collateral ( @{ $self->getAllCollateral( 'variantsJSON' ) } ) {
        ...
        $variants{ $collateral->{ variantId } }
            = join ", ", $collateral->{ shortdesc }, sprintf( '%.2f', $collateral->{ price } );
    }
...
sub getCollateralDataIndex {
    ...
    for ( my $index = 0; $index <= $#{ $table }; $index++ ) {
        return $index if ( exists $table->[ $index ]->{ $keyName }
            and $table->[ $index ]->{ $keyName } eq $keyValue );
    }
...
sub purge {
    my $self    = shift;
    my $sth     = $self->session->db->read( "SELECT
            image1,
            image2,
            image3,
            brochure,
...
sub www_addAccessory {
    ...
    my $accessory       = $self->session->db->buildHashRef( "SELECT
            asset.assetId,
            assetData.title
        FROM
            asset
        ...
        AND
            asset.assetId not in (". $self->session->db->quoteAndJoin( \@usedAccessories ) .")
        AND
            revisionDate = (SELECT max(revisionDate)
                FROM
                    assetData
                WHERE
        ...
        AND
            ...
            OR
            assetData.tagId=". $self->session->db->quote( $self->session->scratch->get( 'versionTag' ) ) .")
        GROUP BY
            assetData.assetId"
    );
...
sub www_addRelated {
    ...
    my $related = $self->session->db->buildHashRef( "SELECT
            asset.assetId,
            assetData.title
        FROM
            asset
        ...
        AND
            asset.assetId
        NOT IN
            (". $self->session->db->quoteAndJoin( \@usedRelated ) .")
        AND
            revisionDate = (SELECT MAX(revisionDate)
                FROM
                    assetData
...
sub view {
    ...
    foreach my $collateral ( @{ $self->getAllCollateral( 'featureJSON' ) } ) {
        ...
        $segment = $self->session->icon->delete('func=deleteFeatureConfirm&fid='.   $id, $self->get( 'url' ),
            $i18n->get( 3 ) )
    ...
    foreach my $collateral ( @{ $self->getAllCollateral( 'benefitJSON' ) } ) {
        ...
        $segment    = $self->session->icon->delete( 'func=deleteBenefitConfirm&bid='.   $id, $self->get( "url" ),
            $i18n->get( 48 ) )
    ...
    foreach my $collateral ( @{ $self->getAllCollateral( 'specificationJSON' ) } ) {
        ...
        $segment    = $self->session->icon->delete( 'func=deleteSpecificationConfirm&sid='. $id,
            $self->get( 'url' ), $i18n->get( 5 ) )
            . $self->session->icon->edit(           'func=editSpecification&sid='
            . $id, $self->get( 'url' ) )
            . $self->session->icon->moveUp(         'func=moveSpecificationUp&sid='
            . $id, $self->get( 'url' ) )
            . $self->session->icon->moveDown(       'func=moveSpecificationDown&sid='
            . $id, $self->get( 'url' ) );
    ...
    foreach my $collateral ( @{ $self->getAllCollateral( 'accessoryJSON' ) } ) {
        ...
        $segment    = $self->session->icon->delete( 'func=deleteAccessoryConfirm&aid='. $id, $self->get( 'url' ),
            $i18n->get( 2 ) )
    ...
    foreach my $collateral ( @{ $self->getAllCollateral( 'relatedJSON' ) } ) {
        ...
        $segment    = $self->session->icon->delete( 'func=deleteRelatedConfirm&rid='.   $id, $self->get( 'url' ),
            $i18n->get( 4 ) )
    ...
    foreach my $collateral ( @{ $self->getAllCollateral( 'variantsJSON' ) } ) {
        ...
        $segment = $self->session->icon->delete(    'func=deleteVariantConfirm&vid='.   $id, $self->get( 'url' ),
            $i18n->get( 'delete variant confirm' ) )
